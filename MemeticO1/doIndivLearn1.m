...
function [ilSols numEval] = doIndivLearn1(baseSols,memParams,data)
...
% Extract error patterns, etc.
%errPats = theSols(:,2:2+numPat-1);
%baseSols = zeros(thisNumSol,numParam+1);
%baseSols(:,1) = theSols(:,1);
%baseSols(:,2:end) = theSols(:,2+numPat:end);
